<template>
  <div class="VueCarousel" :class="{ 'VueCarousel--reverse': paginationPosition === 'top' }">
    <div ref="VueCarousel-wrapper" class="VueCarousel-wrapper h-full">
      <div
        ref="VueCarousel-inner"
        :class="['VueCarousel-inner', { 'VueCarousel-inner--center': isCenterModeEnabled }]"
        :style="{
...
export default {
  ...
  props: {
    ...
    easing: {
      ...
      validator(value) {
        return (
          ["ease", "linear", "ease-in", "ease-out", "ease-in-out"].includes(value) || value.includes("cubic-bezier")
        );
      },
      default: "ease",
    },
  ...
  methods: {
    ...
    getCarouselHeight() {
      ...
      const maxSlideHeight = [...Array(this.currentPerPage)]
        ...
        .reduce((clientHeight, slide) => Math.max(clientHeight, (slide && slide.$el.clientHeight) || 0), 0);

      this.currentHeight = maxSlideHeight === 0 ? "100%" : `${maxSlideHeight}px`;
    ...
    render() {
      ...
      if (this.rtl) {
        this.offset -=
          Math.max(-this.currentPerPage + 1, Math.min(Math.round(this.dragMomentum), this.currentPerPage - 1)) *
          this.slideWidth;
      } else {
        this.offset +=
          Math.max(-this.currentPerPage + 1, Math.min(Math.round(this.dragMomentum), this.currentPerPage - 1)) *
